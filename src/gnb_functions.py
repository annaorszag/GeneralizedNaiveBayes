...
def create_score_matrix(data_train, num):
    ...
    score_matrix[max_row2, max_row] = get_information_content(list(('X' + str(max_row), 'X' + str(max_row2))),
                                                              data_train)
...
def replace_triplet_prob(t0, label, dataframe, vector_realization, var_to_index_dict):
    t0p1_prob = dataframe.loc[(dataframe[t0[0]] == label) \
                              & (dataframe[t0[1]] == vector_realization[var_to_index_dict[t0[1]]])].shape[0] / \
                dataframe.shape[0]
    if t0p1_prob == 0:
        t0p1_prob = replace_pair_prob((t0[0], t0[1]), label, dataframe, vector_realization, var_to_index_dict)
    t0p2_prob = dataframe.loc[(dataframe[t0[0]] == label) \
                              & (dataframe[t0[2]] == vector_realization[var_to_index_dict[t0[2]]])].shape[0] / \
                dataframe.shape[0]
    if t0p2_prob == 0:
        t0p2_prob = replace_pair_prob((t0[0], t0[2]), label, dataframe, vector_realization, var_to_index_dict)
...
def replace_pair_prob(pair, label, dataframe, vector_realization, var_to_index_dict):
    ...
    p2_prob = dataframe.loc[(dataframe[pair[1]] == vector_realization[var_to_index_dict[pair[1]]])].shape[0] / \
              dataframe.shape[0]
...
def get_probability(vector_realization, dataframe, triplet_list, pair_list, label, var_to_index_dict, algorithm):
    ...
    if algorithm == 'GNBA':
        ...
        if t0_prob > 0:
            ...
            t0_prob2 = replace_triplet_prob(t0, label, dataframe, vector_realization, var_to_index_dict)
            t_prob_list.append(t0_prob2)
        for triplet in triplet_list[1:]:
            p_prob = dataframe.loc[(dataframe[triplet[0]] == label) \
                                   & (dataframe[triplet[1]] == vector_realization[
                var_to_index_dict[triplet[1]]])].shape[0] / dataframe.shape[0]
            if p_prob > 0:
                ...
                t_prob = dataframe.loc[(dataframe[triplet[0]] == label) \
                                       ...
                                       & (dataframe[triplet[2]] == vector_realization[
                    var_to_index_dict[triplet[2]]])].shape[0] / dataframe.shape[0]
                if t_prob > 0:
                    t_prob_list.append(t_prob)
                else:
                    t_prob2 = replace_triplet_prob(triplet, label, dataframe, vector_realization, var_to_index_dict)
                    t_prob_list.append(t_prob2)
            else:
                p_prob2 = replace_pair_prob((triplet[0], triplet[1]), label, dataframe, vector_realization, var_to_index_dict)
                t_prob2 = replace_triplet_prob(triplet, label, dataframe, vector_realization, var_to_index_dict)
        ...
        for triplet in triplet_list:
            ...
            if t_prob > 0:
                ...
                t_prob_list.append(t_prob2)
        if len(pair_list) == 0:
            prob = np.prod(t_prob_list)
        else:
            for pair in pair_list:
                p_prob = dataframe.loc[(dataframe[pair[0]] == label) \
                                       & (dataframe[pair[1]] == vector_realization[var_to_index_dict[pair[1]]])].shape[0] / \
                         dataframe.shape[0]
                if p_prob > 0:
                    p_prob_list.append(p_prob)
                else:
                    p_prob2 = replace_pair_prob((pair[0], pair[1]), label, dataframe, vector_realization, var_to_index_dict)
                    p_prob_list.append(p_prob2)
            prob = np.prod(t_prob_list) / np.prod(p_prob_list)
    return prob


def get_probabilities_and_prediction(vector_realization, y, dataframe, triplet_list, pair_list, algorithm, label_pos,
                                     label_neg, var_to_index_dict):
    # p_1
    p_1 = get_probability(vector_realization, dataframe, triplet_list, pair_list, label_pos, var_to_index_dict, algorithm)
    # p_2
    p_2 = get_probability(vector_realization, dataframe, triplet_list, pair_list, label_neg, var_to_index_dict, algorithm)
...
def get_sorted_triple_list(data_train):
    ...
    for step in range(num_steps):
        ...
        max_ic_diff_triplet, max_ic_diff_dict[max_ic_diff_triplet[2]] = max(ic_diff_dict.items(),
...
def test_result(data_test, data_train, V3_list, V2_list, label_pos, label_neg, algorithm, var_to_index_dict):
    data_test_results = pd.DataFrame(
        columns=['label', 'prob_' + str(label_pos), 'prob_' + str(label_neg), 'prediction'])
    for index, row in data_test.iterrows():
        y = np.array([row['Y']])
        vector = np.array(row[1:])
        pred_row = get_probabilities_and_prediction(vector, y, data_train, V3_list, V2_list, algorithm, label_pos,
                                                    label_neg, var_to_index_dict)
...
def test_results(data_test, data_train, V3_list, V2_list, label_pos, label_neg, algorithm, var_to_index_dict, num_cols):
    accuracy_list = []
    precision_list = []
    recall_list = []
    auc_list = []
    for num_of_triplets in range(1, num_cols - 1):
        ...
        for index, row in data_test.iterrows():
            ...
            if algorithm == "GNBO":
                pred_row = get_probabilities_and_prediction(vector, y, data_train, V3_list[:num_of_triplets],
                                                            V2_list[:(num_of_triplets - 1)], algorithm,
                                                            label_pos, label_neg, var_to_index_dict)
            elif algorithm == "GNBA":
                pred_row = get_probabilities_and_prediction(vector, y, data_train, V3_list[:num_of_triplets],
                                                            None, algorithm,
                                                            label_pos, label_neg, var_to_index_dict)
